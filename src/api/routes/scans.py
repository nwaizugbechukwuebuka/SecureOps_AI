...
class ScanRequest(BaseModel):
    ...
    notify_on_completion: bool = Field(
        default=True,
        description="Whether to send notifications when scan completes"
    )
...
async def initiate_security_scan(
    ...
    """
    try:
        logger.info(f"Initiating security scan for {scan_request.repository_url}#{scan_request.branch} by user {current_user.id}")
...
@router.get("/scanners/health", response_model=ScannerHealthResponse, tags=["Scanner Management"])
