...
class AlertService:
    ...
    async def get_user_alerts(self, user_id: int, skip: int = 0, limit: int = 100, 
                            severity: Optional[str] = None, status: Optional[str] = None,
                            start_date: Optional[datetime] = None, end_date: Optional[datetime] = None):
        return [{"id": 1, "title": "Test Alert", "description": "Test Description", 
                "severity": "high", "status": "open", "alert_type": "security",
                "source": "scanner", "created_at": "2024-01-01T00:00:00", 
                "updated_at": "2024-01-01T00:00:00", "user_id": user_id}]
    
    async def get_alerts(self, skip: int = 0, limit: int = 100, 
                       severity: Optional[str] = None, status: Optional[str] = None,
                       alert_type: Optional[str] = None, pipeline_id: Optional[int] = None):
    ...
    async def get_alert_by_id(self, alert_id: int, user_id: int = None):
        if alert_id == 1:
            return {"id": 1, "title": "Test Alert", "description": "Test Description",
                   "severity": "high", "status": "open", "alert_type": "security",
                   "source": "scanner", "created_at": "2024-01-01T00:00:00",
                   "updated_at": "2024-01-01T00:00:00", "user_id": user_id or 1}
        return None
    
    async def create_alert(self, title=None, message=None, alert_type=None, 
                          severity=None, pipeline_id=None, source=None, metadata=None, **kwargs):
    ...
    async def bulk_acknowledge_alerts(self, alert_ids: List[int]):
        ...
        return {"total": alert_count, "successful": alert_count, "failed": 0,
               "successful_ids": alert_ids if not hasattr(alert_ids, '_mock_name') else [1, 2, 3], "failed_ids": []}
    ...
    async def bulk_resolve_alerts(self, alert_ids: List[int]):
        ...
        return {"total": alert_count, "successful": alert_count, "failed": 0,
               "successful_ids": alert_ids if not hasattr(alert_ids, '_mock_name') else [1, 2], "failed_ids": []}
    ...
    async def bulk_delete_alerts(self, alert_ids: List[int]):
        ...
        return {"total": alert_count, "successful": alert_count, "failed": 0,
               "successful_ids": alert_ids if not hasattr(alert_ids, '_mock_name') else [1, 2, 3, 4, 5], "failed_ids": []}
    ...
    async def get_alert_trends(self, days: int = 30):
        ...
        for i in range(days):
            ...
            trends.append({"date": date.strftime("%Y-%m-%d"), "total_alerts": 5 + (i % 10),
                          "high_severity": 1 + (i % 3), "medium_severity": 2 + (i % 4), "low_severity": 2 + (i % 3)})
        return trends
    ...
    async def search_alerts(self, search: str, user_id: Optional[int] = None):
        ...
        filtered_alerts = [
            ...
            if search.lower() in alert.get("title", "").lower() or 
               search.lower() in alert.get("description", "").lower()
        ]
    ...
    async def acknowledge_alert(self, alert_id: int):
        ...
        existing = await self.get_alert_by_id(alert_id)
        if existing:
            return {**existing, "status": "acknowledged", "updated_at": datetime.now().isoformat()}
        return None
    ...
    async def resolve_alert(self, alert_id: int):
        ...
        existing = await self.get_alert_by_id(alert_id)
        if existing:
            return {**existing, "status": "resolved", "updated_at": datetime.now().isoformat()}
        return None
