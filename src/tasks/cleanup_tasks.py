...
@celery_app.task(
    bind=True,
    name="secureops.tasks.cleanup_tasks.cleanup_old_scan_jobs",
    max_retries=2
)
def cleanup_old_scan_jobs(
    ...
    try:
        result = asyncio.run(_cleanup_old_scan_jobs_async(
            retention_days, 
            batch_size, 
            cleanup_id
        ))
...
def cleanup_temporary_files(
    ...
    try:
        ...
        logger.error(f"[{cleanup_id}] Temporary file cleanup failed: {str(e)}", extra=error_details)
...
def archive_old_vulnerabilities(
    ...
    try:
        result = asyncio.run(_archive_old_vulnerabilities_async(
            retention_days,
            batch_size,
            cleanup_id
        ))
        ...
        logger.error(f"[{cleanup_id}] Vulnerability archival failed: {str(e)}", extra=error_details)
...
@celery_app.task(
    bind=True,
    name="secureops.tasks.cleanup_tasks.cleanup_log_files",
    max_retries=2
)
...
@celery_app.task(
    bind=True,
    name="secureops.tasks.cleanup_tasks.database_maintenance",
    max_retries=1
)
def database_maintenance(self) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"[{maintenance_id}] Database maintenance failed: {str(e)}", extra=error_details)
...
async def _comprehensive_cleanup_async(cleanup_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        cleanup_tasks = [
            ("scan_jobs", _cleanup_old_scan_jobs_async(DEFAULT_RETENTION_DAYS, MAX_CLEANUP_BATCH_SIZE, cleanup_id)),
            ("temp_files", _cleanup_temporary_files_async(TEMP_FILE_MAX_AGE_HOURS, cleanup_id)),
            ("vulnerabilities", _archive_old_vulnerabilities_async(90, MAX_CLEANUP_BATCH_SIZE, cleanup_id)),
        ...
        for task_name, task_coro in cleanup_tasks:
            try:
                ...
                # Aggregate metrics
                results["total_items_cleaned"] += task_result.get("deleted_count", 0) + task_result.get("archived_count", 0)
