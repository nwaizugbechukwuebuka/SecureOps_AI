...
def test_audit_log_on_login(monkeypatch):
    ...
    from src.api.utils.logger import AuditLogger
    logs = []
    class DummyLogger:
    ...
    audit_logger.login_attempt(username="alice", success=False, ip_address="1.2.3.4", failure_reason="bad password")
    assert any("login_attempt" in args for args, _ in logs)
