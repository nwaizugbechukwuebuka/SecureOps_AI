...
class EmailNotificationService:
    ...
    def send_security_alert(
        self, recipients: List[str], alert_title: str, alert_description: str, severity: str, source: str = None
    ) -> bool:
...
"""
        ...
        # Create HTML body
        severity_color = {"critical": "#dc2626", "high": "#ea580c", "medium": "#d97706", "low": "#65a30d"}.get(
...
class SlackNotificationService:
    ...
    def send_security_alert(self, alert_title: str, alert_description: str, severity: str, source: str = None) -> bool:
        ...
        attachment = {
            ...
            "fields": [
                ...
                {"title": "Description", "value": alert_description, "short": False},
                {"title": "Time", "value": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"), "short": True},
            ],
        }

        if source:
            attachment["fields"].append({"title": "Source", "value": source, "short": True})
...
class NotificationManager:
    ...
    async def send_security_alert(
        ...
        if recipients and recipients.get("email"):
            task = asyncio.create_task(
                self._send_email_alert(recipients["email"], alert_title, alert_description, severity, source)
            )
            tasks.append(task)

        # Send Slack notifications
        if recipients and recipients.get("slack") or settings.ENABLE_SLACK_NOTIFICATIONS:
            task = asyncio.create_task(self._send_slack_alert(alert_title, alert_description, severity, source))
            tasks.append(task)
    ...
    async def _send_email_alert(self, recipients, title, description, severity, source):
        ...
        try:
            ...
            await loop.run_in_executor(
                None, self.email_service.send_security_alert, recipients, title, description, severity, source
            )
        except Exception as e:
    ...
    async def _send_slack_alert(self, title, description, severity, source):
        ...
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
                None, self.slack_service.send_security_alert, title, description, severity, source
            )
        except Exception as e:
            print(f"Failed to send Slack alert: {str(e)}")

    def send_user_notification(
        self, user_email: str, subject: str, message: str, notification_type: str = "info"
    ) -> bool:
        ...
        return self.email_service.send_email([user_email], email_subject, message)
