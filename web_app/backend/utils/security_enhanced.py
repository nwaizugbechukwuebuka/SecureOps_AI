...
class MFAService:
    ...
    def get_totp_uri(secret: str, email: str) -> str:
        ...
        return totp.provisioning_uri(email, issuer_name=SecurityConfig.MFA_ISSUER)
...
class TokenManager:
    ...
    def create_refresh_token(user_id: int, jti: str = None) -> str:
        ...
        payload = {"sub": str(user_id), "exp": expire, "iat": datetime.now(timezone.utc), "jti": jti, "type": "refresh"}
    ...
    def verify_token(token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            payload = jwt.decode(token, settings.jwt_secret_key, algorithms=[SecurityConfig.ALGORITHM])
            return payload
...
class RateLimiter:
    ...
    def check_rate_limit(db: Session, ip_address: str, endpoint: str) -> bool:
        ...
        rate_limit = (
            db.query(RateLimit).filter(RateLimit.ip_address == ip_address, RateLimit.endpoint == endpoint).first()
        )

        if not rate_limit:
            # First request from this IP for this endpoint
            rate_limit = RateLimit(ip_address=ip_address, endpoint=endpoint, attempts=1, window_start=now)
            db.add(rate_limit)
            db.commit()
            return True
...
async def get_current_user(
    ...
    jti = payload.get("jti")
    if jti:
        session = db.query(UserSession).filter(UserSession.token_jti == jti, UserSession.user_id == user.id).first()
