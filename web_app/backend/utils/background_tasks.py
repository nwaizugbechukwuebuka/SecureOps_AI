...
class BackgroundTaskManager:
    ...
    async def start(self):
        ...
        self.tasks['system_health_check'] = asyncio.create_task(
            self.periodic_system_health_check()
        )
        self.tasks['cleanup_old_logs'] = asyncio.create_task(
            self.periodic_cleanup()
        )
        self.tasks['alert_digest'] = asyncio.create_task(
            self.periodic_alert_digest()
        )
    ...
    async def cleanup_old_data(self):
        ...
        try:
            ...
            old_events = db.query(SecurityEvent).filter(
                SecurityEvent.timestamp < event_cutoff_date
            )
    ...
    async def create_system_alert(
        ...
        try:
            ...
            security_logger.log_security_event(
                ...
                severity,
                title,
                {"source": source, "alert_id": alert.id}
            )
