...
async def demonstrate_security_features():
    ...
    try:
        ...
        test_passwords = ["weak", "StrongPass123!", "VerySecurePassword2024@#"]

        for password in test_passwords:
            validation = password_validator.validate_password(password)
            strength_color = "🔴" if validation["strength"] in ["weak", "medium"] else "🟢"
            print(
                f"  {strength_color} '{password}': {validation['strength'].upper()} - Valid: {validation['is_valid']}"
            )
        print()
        ...
        try:
            # Create a test token
            test_user_data = {"user_id": 1, "username": "admin", "role": "admin"}
        ...
        try:
            await security_logger.log_user_login(
                user_id=1, username="admin", ip_address="127.0.0.1", user_agent="Demo Script", success=True
            )
