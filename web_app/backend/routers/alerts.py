...
async def get_alert(
    alert_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)
):
    """
...
async def create_alert(
    alert: AlertCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)
):
    """
...
async def delete_alert(
    ...
    if current_user.role != "admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only administrators can delete alerts")
...
async def get_alerts_summary(
    ...
    alerts = db.query(Alert).filter(Alert.created_at >= start_date, Alert.created_at <= end_date).all()
