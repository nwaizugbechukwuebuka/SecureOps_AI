...
async def get_dashboard_stats(
    ...
    # Login statistics
    successful_logins_24h = db.query(AuditLog).filter(
    ...
    # MFA adoption rate
    mfa_users = db.query(User).filter(
...
async def get_login_activity(
    ...
    return {
        ...
        "failed_logins_timeline": [
            {"hour": item.hour.isoformat(), "count": item.count}
            for item in failed_logins
        ],
        "top_failed_login_ips": [
            {"ip_address": item.ip_address, "attempts": item.count}
...
async def get_user_activity(
    ...
    activity_by_type = db.query(
        ...
        AuditLog.created_at >= since
    ).group_by(AuditLog.event_type).order_by(desc('count')).all()
...
async def get_security_overview(
    ...
    recent_password_changes = db.query(AuditLog).filter(
    ...
    recent_role_changes = db.query(AuditLog).filter(
        AuditLog.event_type == AuditEventType.ROLE_CHANGE.value,
        AuditLog.created_at >= last_7d
    ).count()
    
    # Critical events in last 24h
    critical_events_24h = db.query(AuditLog).filter(
        AuditLog.risk_level == 'CRITICAL',
        AuditLog.created_at >= last_24h
    ).count()
    
    # Failed login attempts by IP
    failed_login_ips = db.query(
        AuditLog.ip_address,
    ...
    return {
        ...
        "threat_indicators": {
            ...
            "suspicious_ips": len(failed_login_ips),
            "failed_login_ips": [
                {"ip": ip.ip_address, "attempts": ip.attempts}
                for ip in failed_login_ips
            ]
        },
        "security_score": {
            "score": round(security_score, 1),
            "level": "EXCELLENT" if security_score >= 90 else
                    "GOOD" if security_score >= 75 else
                    "FAIR" if security_score >= 60 else "POOR",
