...
async def login(credentials: UserLogin, db: Session = Depends(get_db)):
    ...
    if not user:
        ...
        log_security_event(
            ...
            {"username": credentials.username, "reason": "user_not_found"}
        )
        raise HTTPException(
    ...
    if not user.verify_password(credentials.password):
        ...
        log_security_event(
            "failed_login_attempt", 
            {"username": credentials.username, "reason": "invalid_password"},
            user_id=user.id
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if not user.is_active:
        log_security_event(
            "failed_login_attempt", 
            {"username": credentials.username, "reason": "account_inactive"},
            user_id=user.id
        )
        raise inactive_user_exception
