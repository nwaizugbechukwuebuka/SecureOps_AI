...
from utils.security_enhanced import (
    get_current_user, require_admin, require_analyst, PasswordValidator,
...
async def get_available_roles(current_user: User = Depends(require_analyst)):
    ...
    return {
        "roles": [
            {"value": UserRole.VIEWER.value, "label": "Viewer", "description": "Read-only access to dashboards and alerts"},
            {"value": UserRole.ANALYST.value, "label": "Analyst", "description": "Can view and analyze security data, manage alerts"},
            {"value": UserRole.ADMIN.value, "label": "Administrator", "description": "Full system access including user management"}
        ]
    }
...
async def get_user_audit_logs(
    ...
    security_logger.log_data_access(
        db=db,
        user=current_user,
        resource=f"audit_logs:user:{user_id}",
        ip_address=client_ip
    )
...
async def get_user_statistics(
    ...
    # MFA adoption
    mfa_enabled_count = db.query(User).filter(
    ...
    recent_logins = db.query(func.count(func.distinct(AuditLog.user_id))).filter(
    ...
    return {
        ...
        "recent_activity": {
            "unique_logins_24h": recent_logins,
