...
async def get_dashboard_stats(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    security_events_today = (
        db.query(func.count(SecurityEvent.id)).filter(SecurityEvent.timestamp >= since_yesterday).scalar() or 0
    )

    critical_events_today = (
        db.query(func.count(SecurityEvent.id))
        .filter(and_(SecurityEvent.timestamp >= since_yesterday, SecurityEvent.severity == "critical"))
        .scalar()
        or 0
    )

    # Count automation tasks by status
    automation_tasks_running = (
        db.query(func.count(AutomationTask.id)).filter(AutomationTask.status == "running").scalar() or 0
    )
    ...
    unread_notifications = (
        ...
        .filter(and_(Notification.user_id == current_user.get("user_id"), ~Notification.read))
        .scalar()
        or 0
    )
...
async def get_notifications(
    ...
    query = db.query(Notification).filter(Notification.user_id == current_user.get("user_id"))
...
async def mark_notification_read(
    notification_id: int, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Mark notification as read"""

    notification = (
        db.query(Notification)
        .filter(and_(Notification.id == notification_id, Notification.user_id == current_user.get("user_id")))
        .first()
    )
...
async def mark_all_notifications_read(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """Mark all notifications as read for current user"""

    db.query(Notification).filter(and_(Notification.user_id == current_user.get("user_id"), ~Notification.read)).update(
        {Notification.read: True}
    )
...
async def get_recent_activity(
    limit: int = Query(10, ge=1, le=50), current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Get recent system activity summary"""

    # Get recent security events
    recent_events = db.query(SecurityEvent).order_by(desc(SecurityEvent.timestamp)).limit(limit).all()

    # Get recent automation tasks
    recent_tasks = db.query(AutomationTask).order_by(desc(AutomationTask.created_at)).limit(limit).all()
    ...
    return {"activities": activity_items[:limit], "total_items": len(activity_items)}


@router.post("/test-notification")
async def create_test_notification(
    background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    log_security_event(
        "test_notification_created", {"username": current_user.get("username")}, user_id=current_user.get("user_id")
    )
