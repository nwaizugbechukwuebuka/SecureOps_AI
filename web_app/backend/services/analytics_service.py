...
class AnalyticsService:
    ...
    def get_security_overview(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            alerts = self.db.query(Alert).filter(Alert.created_at >= start_date, Alert.created_at <= end_date).all()

            # Get security events in date range
            events = (
                self.db.query(SecurityEvent)
                .filter(SecurityEvent.timestamp >= start_date, SecurityEvent.timestamp <= end_date)
                .all()
            )
            ...
            previous_alerts = (
                self.db.query(Alert)
                .filter(Alert.created_at >= previous_period_start, Alert.created_at < start_date)
                .count()
            )
    ...
    def get_daily_alert_trends(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            daily_data = defaultdict(
                lambda: {"date": None, "total": 0, "critical": 0, "high": 0, "medium": 0, "low": 0}
            )
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_alerts_24h = (
                self.db.query(Alert)
                .filter(Alert.severity == "critical", Alert.created_at >= now - timedelta(hours=24))
                .count()
            )
            ...
            active_users = self.db.query(User).filter(User.last_login >= now - timedelta(days=30)).count()

            # Alerts by status in last 7 days
            week_ago = now - timedelta(days=7)
            week_alerts = self.db.query(Alert).filter(Alert.created_at >= week_ago).all()
    ...
    def get_top_threats(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            alerts = self.db.query(Alert).filter(Alert.created_at >= thirty_days_ago).all()

            # Group by source and calculate threat scores
            threat_data = defaultdict(
                lambda: {
                    "source": None,
                    "alert_count": 0,
            ...
            for source, data in threat_data.items():
                ...
                threat_score = sum(
                    count * severity_weights[severity] for severity, count in data["severity_scores"].items()
                )
    ...
    def _calculate_threat_score(self, pattern_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            severity_multiplier = sum(
                count * severity_weights.get(severity, 1.0) for severity, count in pattern_data["severities"].items()
