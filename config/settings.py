...
class Settings(BaseSettings):
    ...
    database_url: str = Field(default_factory=lambda: os.getenv("DATABASE_URL", "sqlite:///./secureops.db"))
    ...
    # ====== SECURITY SETTINGS ======
    jwt_secret_key: str = Field(default_factory=lambda: os.getenv("JWT_SECRET_KEY", "jwt-secret-key-change-me"))
    jwt_algorithm: str = Field(default="HS256")
    jwt_access_token_expire_minutes: int = Field(default=60)
    jwt_refresh_token_expire_days: int = Field(default=7)
    password_min_length: int = Field(default=8)
    bcrypt_rounds: int = Field(default=12)
    session_secret_key: str = Field(default_factory=lambda: os.getenv("SESSION_SECRET_KEY", "session-secret-change-me"))
    ...
    compliance_frameworks: List[str] = Field(default=["OWASP", "CIS", "NIST", "SOX", "HIPAA", "GDPR"])
